...
namespace Files.InteractionTests.Tests
{
	...
	public sealed class ContextMenuTests
	{
		...
		public void TestWidgetContextMenus()
		{
			...
			ExpandSubMenuIfPresent("Send to", () => TestHelper.ContextClickElement(pinnedCard));
		...
		public void TestSidebarContextMenus()
		{
			...
			TestHelper.WaitForElementByName("Open in new tab");
			AxeHelper.AssertNoAccessibilityErrors();
			ExpandSubMenuIfPresent("Send to", () => TestHelper.ContextClickElementById("Desktop"));
		...
		public void TestOpenWithAndSendToSubMenus()
		{
			...
			AssertSubMenuLoadsItems("Open with", () => TestHelper.ContextClickElementByName(fileName));

			// Expanding Send to must reveal at least one loaded target entry
			AssertSubMenuLoadsItems("Send to", () => TestHelper.ContextClickElementByName(fileName));
		...
		/// brings it back.
		/// </summary>
		private static void AssertSubMenuLoadsItems(string subMenuName, Action reopenMenu = null)
		{
			...
			while (DateTime.UtcNow < deadline)
			{
				...
				// When the shell offers no sub-items for the item type (common on bare CI images),
				// the app shows the leaf form of Open with / Send to instead of a submenu. There is
				// nothing to expand then, and clicking the leaf would launch a dialog instead.
				if (TestHelper.IsLeafMenuItem(subMenuName))
					return;
		...
		/// </summary>
		private static void ExpandSubMenuIfPresent(string subMenuName, Action reopenMenu = null)
		{
			if (TestHelper.ElementExistsByName(subMenuName, 700))
				AssertSubMenuLoadsItems(subMenuName, reopenMenu);
		}
