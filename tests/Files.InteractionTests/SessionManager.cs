		...
		public static void CreateSession(TestContext _)
		{
			if (_session is null)
			{
				...
				if (_session is null)
				{
					...
					if (File.Exists($@"{Environment.GetEnvironmentVariable("SystemDrive")}\Program Files (x86)\Windows Application Driver\WinAppDriver.exe"))
					{
						Process.Start($@"{Environment.GetEnvironmentVariable("SystemDrive")}\Program Files (x86)\Windows Application Driver\WinAppDriver.exe");
					}
					else if (File.Exists($@"{Environment.GetEnvironmentVariable("SystemDrive")}\Program Files\Windows Application Driver\WinAppDriver.exe"))
					{
						Process.Start($@"{Environment.GetEnvironmentVariable("SystemDrive")}\Program Files\Windows Application Driver\WinAppDriver.exe");
					}
					else
					{
