...
namespace Files.App.Shell
{
	public static class ShellFolderExtensions
	{
		...
		public static ShellFileItem GetShellFileItem(ShellItem folderItem)
		{
			...
			string filePath = string.IsNullOrEmpty(Path.GetDirectoryName(parsingPath))
				// Null if root
				? parsingPath
				// In recycle bin "Name" contains original file path + name
				: Path.Combine(Path.GetDirectoryName(parsingPath), itemNameOrOriginalPath);

			if (!isFolder && !string.IsNullOrEmpty(parsingPath) &&
				Path.GetExtension(parsingPath) is string realExtension &&
				!string.IsNullOrEmpty(realExtension))
			{
				if (!string.IsNullOrEmpty(fileName) &&
					!fileName.EndsWith(realExtension, StringComparison.OrdinalIgnoreCase))
				{
					fileName = $"{fileName}{realExtension}";
				}
				if (!string.IsNullOrEmpty(filePath) &&
					!filePath.EndsWith(realExtension, StringComparison.OrdinalIgnoreCase))
				{
			...
			// This is LocalTime
			var recycleDate = fileTime?.ToDateTime().ToLocalTime() ?? DateTime.Now;
			fileTime = folderItem.Properties.TryGetProperty<System.Runtime.InteropServices.ComTypes.FILETIME?>(Ole32.PROPERTYKEY.System.DateModified);

			// This is LocalTime
			var modifiedDate = fileTime?.ToDateTime().ToLocalTime() ?? folderItem.FileInfo?.LastWriteTime ?? DateTime.Now;
			fileTime = folderItem.Properties.TryGetProperty<System.Runtime.InteropServices.ComTypes.FILETIME?>(Ole32.PROPERTYKEY.System.DateCreated);

			// This is LocalTime
			var createdDate = fileTime?.ToDateTime().ToLocalTime() ?? folderItem.FileInfo?.CreationTime ?? DateTime.Now;
