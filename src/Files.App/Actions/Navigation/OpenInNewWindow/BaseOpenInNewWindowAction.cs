...
namespace Files.App.Actions
{
	internal abstract class BaseOpenInNewWindowAction : ObservableObject, IAction
	{
		...
		public virtual bool IsExecutable =>
			...
			ContentPageContext.ShellPage.LayoutPage is not null &&
		...
		{
			if (ContentPageContext.ShellPage?.LayoutPage?.SelectedItems is null)
				return;

			List<StandardStorageItem> items = ContentPageContext.ShellPage.LayoutPage.SelectedItems;
