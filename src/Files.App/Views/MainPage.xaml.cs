...
namespace Files.App.Views
{
	public sealed partial class MainPage : Page
	{
		...
		public IDevToolsSettingsService DevToolsSettingsService { get; } = Ioc.Default.GetRequiredService<IDevToolsSettingsService>();
		...
		private void UserSettingsService_OnSettingChangedEvent(object? sender, SettingChangedEventArgs e)
		{
			switch (e.SettingName)
			{
				...
				case nameof(IAppearanceSettingsService.SidebarWidth):
					ApplySidebarWidthState();
					ApplyTreeViewSidebarCompactMode();
					break;
			}
		}
		...
		public object? GetEffectiveMenuItemsSourceResponsive(bool showTreeViewSidebar, Files.App.Controls.SidebarDisplayMode displayMode, bool isPaneOpen, double sidebarWidth)
		{
			...
			var treeViewVisible = GetTreeViewSidebarVisibility(displayMode, isPaneOpen, sidebarWidth) == Visibility.Visible;
			return treeViewVisible ? null : SidebarAdaptiveViewModel.SidebarItems;
		}

		// When the user drags the sidebar resizer below the threshold, force SidebarView to Compact mode so the original sidebar renders as icons-only. Above the threshold (and outside Minimal mode), restore Expanded mode so the TreeView can take over.
		private void ApplyTreeViewSidebarCompactMode()
		{
			if (SidebarAdaptiveViewModel is null || SidebarControl is null)
				return;
