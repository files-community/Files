	...
	public sealed partial class ColumnsLayoutPage : BaseLayoutPage
	{
		...
		private void ColumnViewBase_ItemInvoked(object? sender, EventArgs e)
		{
			...
			if (nextBlade is null || arePathsDifferent)
			{
				...
				ParentShellPage.TabBarItemParameter.NavigationParameter = column.NavPathParam;
			}
		}

		private void ContentChanged(IShellPage p)
		{
			(ParentShellPage as ModernShellPage)?.RaiseContentChanged(p, p.TabBarItemParameter);
		}
		...
		public override void Dispose()
		{
			...
			foreach (var frame in columnHostItems)
			{
				if (frame?.Content is ColumnShellPage shPage)
				{
					...
					if (shPage.LayoutPage is ColumnLayoutPage viewBase)
					{
			...
			if (index >= 0)
			{
				SafetyExtensions.IgnoreExceptions(() =>
				{
					while (ColumnHost.ActiveBlades.Count > index + 1)
					{
						...
						if ((frame?.Content as ColumnShellPage)?.LayoutPage is ColumnLayoutPage columnLayout)
						{
					...
					if ((ColumnHost.ActiveBlades[index].Content as Frame)?.Content is ColumnShellPage s)
					{
						...
						ParentShellPage.TabBarItemParameter.NavigationParameter = s.ShellViewModel.WorkingDirectory;
					}
				});
			}
		...
		private void ColumnViewBrowser_ContentChanged(object sender, TabBarItemParameter e)
		{
			...
			var columnView = c?.LayoutPage as ColumnLayoutPage;
		...
		private void ColumnViewBase_KeyUp(object sender, Microsoft.UI.Xaml.Input.KeyRoutedEventArgs e)
		{
			...
			if (shPage?.LayoutPage?.SelectedItem?.PrimaryItemAttribute is not StorageItemTypes.Folder)
				CloseUnnecessaryColumns(shPage?.ColumnParams);
		}

		public void NavigateBack()
		{
			(ParentShellPage as ModernShellPage)?.Back_Click();
		}

		public void NavigateForward()
		{
			(ParentShellPage as ModernShellPage)?.Forward_Click();
		}
		...
		public void NavigateUp()
		{
			...
			else
			{
				...
				if (workingDirectory is null || string.Equals(workingDirectory, GetPathRoot(workingDirectory), StringComparison.OrdinalIgnoreCase))
					ParentShellPage?.NavigateHome();
				else
					ParentShellPage?.NavigateToPath(GetParentDir(workingDirectory));
			}
		}
		...
		private ColumnLayoutPage? RetrieveBladeColumnViewBase(BladeItem blade)
		{
			...
			return activeBladePage.LayoutPage as ColumnLayoutPage;
		}
		...
		public void SetSelectedPathOrNavigate(string navigationPath, Type sourcePageType, NavigationArguments navArgs = null)
		{
			...
			{
				ParentShellPage?.NavigateToPath(navArgs.SearchPathParam, typeof(DetailsLayoutPage), navArgs);
				return;
			}
			...
			{
				ParentShellPage?.NavigateToPath(navigationPath, sourcePageType, navArgs);
				return;
			}
		...
		public void SetSelectedPathOrNavigate(PathNavigationEventArgs e)
		{
			...
			if (NormalizePath(ParentShellPage.ShellViewModel?.WorkingDirectory) != NormalizePath(e.ItemPath))
				ParentShellPage.NavigateToPath(e.ItemPath);
			else
