...
namespace Files.App.Views.Layouts
{
	...
	public abstract class BaseLayoutPage : Page, IBaseLayoutPage, INotifyPropertyChanged
	{
		...
		public LayoutPreferencesManager? FolderSettings
			=> ParentShellPageInstance?.ShellInstanceViewModel.FolderSettings;

		public ShellInstanceViewModel? InstanceViewModel
			=> ParentShellPageInstance?.ShellInstanceViewModel;
		...
		protected override async void OnNavigatedTo(NavigationEventArgs e)
		{
			...
			if (!navigationArguments.IsSearchResultPage)
			{
				...
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeRecycleBin = isRecycleBin;
				...
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeMtpDevice = workingDir.StartsWith("\\\\?\\", StringComparison.Ordinal);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeFtp = FtpHelpers.IsFtpPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeZipFolder = ZipStorageFolder.IsZipPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeLibrary = LibraryManager.IsLibraryPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeSearchResults = false;
				ParentShellPageInstance.ToolbarViewModel.PathControlDisplayText = navigationArguments.NavPathParam;

				if (ParentShellPageInstance.ShellInstanceViewModel.FolderSettings.DirectorySortOption == SortOption.Path)
					ParentShellPageInstance.ShellInstanceViewModel.FolderSettings.DirectorySortOption = SortOption.Name;

				if (ParentShellPageInstance.ShellInstanceViewModel.FolderSettings.DirectoryGroupOption == GroupOption.FolderPath &&
					!ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeLibrary)
					ParentShellPageInstance.ShellInstanceViewModel.FolderSettings.DirectoryGroupOption = GroupOption.None;
			...
			else
			{
				...
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeRecycleBin = workingDir.StartsWith(Constants.UserEnvironmentPaths.RecycleBinPath, StringComparison.Ordinal);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeMtpDevice = workingDir.StartsWith("\\\\?\\", StringComparison.Ordinal);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeFtp = FtpHelpers.IsFtpPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeZipFolder = ZipStorageFolder.IsZipPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeLibrary = LibraryManager.IsLibraryPath(workingDir);
				ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeSearchResults = true;
			...
			ParentShellPageInstance.ShellInstanceViewModel.IsPageTypeNotHome = true;
		...
		private void RefreshItem(SelectorItem container, object item, bool inRecycleQueue, ContainerContentChangingEventArgs args)
		{
			...
			else
			{
				...
				if (!listedItem.ItemPropertiesInitialized)
				{
					...
					{
						await ParentShellPageInstance!.ShellViewModel.LoadExtendedItemPropertiesAsync(listedItem, IconSize);
