...
namespace Files.App.Helpers
{
	public static class ShellContextmenuHelper
	{
		...
		public static async Task<List<CustomMenuFlyoutItem>> GetShellContextmenuAsync(bool showOpenMenu, bool shiftPressed, string workingDirectory, List<ListedItem>? selectedItems, CancellationToken cancellationToken)
		{
			bool IsItemSelected = selectedItems?.Count > 0;

			var menuItemsList = new List<CustomMenuFlyoutItem>();
		...
		private static void LoadMenuFlyoutItem(IList<CustomMenuFlyoutItem> menuItemsListLocal,
								...
								int itemsBeforeOverflow = int.MaxValue)
		{
			...
			if (overflowItems.Where(x => x.Type != MenuItemType.MFT_SEPARATOR).Any())
			{
				...
				if (moreItem is null)
				{
					var menuLayoutSubItem = new CustomMenuFlyoutItem()
					{
				...
				.SkipWhile(x => x.Type == MenuItemType.MFT_SEPARATOR)) // Remove trailing separators
			{
				...
				{
					var menuLayoutItem = new CustomMenuFlyoutItem()
					{
		...
		public static List<CustomMenuFlyoutItem>? GetOpenWithItems(List<CustomMenuFlyoutItem> flyout)
		{
		...
		public static List<CustomMenuFlyoutItem>? GetSendToItems(List<CustomMenuFlyoutItem> flyout)
		{
			...
			bool showSendToMenu = false)
		{
			try
			{
				...
				var shellMenuItems = await ContentContextFlyoutFactory.GetItemContextShellCommandsAsync(
					workingDir: null,
				...
				{
					var (_, secondaryElements) = ItemToCommandBarConversionHelper.GetAppBarItemsFromModel(shellMenuItems);
				...
				{
					var overflowItems = ItemToCommandBarConversionHelper.GetMenuFlyoutItemsFromModel(shellMenuItems);
				...
				if (openWithItem?.LoadSubMenuAction is not null)
				{
					...
					var (_, openWithItems) = ItemToCommandBarConversionHelper.GetAppBarItemsFromModel(new List<CustomMenuFlyoutItem>() { openWithItem });
				...
				if (sendToItem?.LoadSubMenuAction is not null)
				{
					...
					var (_, sendToItems) = ItemToCommandBarConversionHelper.GetAppBarItemsFromModel(new List<CustomMenuFlyoutItem>() { sendToItem });
		...
		public static void AddItemsToMainMenu(IEnumerable<ICommandBarElement> mainMenu, CustomMenuFlyoutItem viewModel)
		{
			...
			if (appBarButton is not null)
			{
				...
				ItemToCommandBarConversionHelper.GetMenuFlyoutItemsFromModel(viewModel.Items)?.ForEach(i => ctxFlyout.Items.Add(i));
		...
		public static void AddItemsToOverflowMenu(AppBarButton? overflowItem, CustomMenuFlyoutItem viewModel)
		{
			if (overflowItem?.Flyout is MenuFlyout flyout)
			{
				...
				{
					viewModel.Items.ForEach(i => flyoutSubItem.Items.Add(ItemToCommandBarConversionHelper.GetMenuItem(i)));
