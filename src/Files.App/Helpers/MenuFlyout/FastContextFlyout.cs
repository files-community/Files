...
namespace Files.App.Helpers.ContextFlyouts
{
	...
	public sealed class FastContextFlyout
	{
		...
		/// <summary>
		/// Resolves the open direction (predicted from the invocation point and the estimated menu height) and
		/// positions the primary row accordingly. Call after building, BEFORE the menu is shown.
		/// </summary>
		...
		/// When the shell yields no sub-items (e.g. no apps registered for the type), <paramref name="onEmpty"/>
		/// runs instead so the caller can revert to its leaf/placeholder form rather than show an empty submenu.
		/// </summary>
		...
		// The screen-physical point the menu opens from. WinUI anchors a pointer-invoked context flyout at
		// the pointer, so the live cursor position is the reference whenever the cursor is inside the anchor
		// element; the anchor's top covers keyboard invocation, and the cursor alone covers large containers
		// (the background menu's whole file area). NaN when unknown.
