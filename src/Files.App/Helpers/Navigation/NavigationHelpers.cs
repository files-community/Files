...
namespace Files.App.Helpers
{
	public static class NavigationHelpers
	{
		...
		public static async Task<bool> OpenPath(string path, IShellPage associatedInstance, FilesystemItemType? itemType = null, bool openSilent = false, bool openViaApplicationPicker = false, IEnumerable<string>? selectItems = null, string? args = default, bool forceOpenInNewTab = false)
		{
			...
			bool isHiddenItem = InteropHelper.HasFileAttribute(path, System.IO.FileAttributes.Hidden);
			bool isDirectory = InteropHelper.HasFileAttribute(path, System.IO.FileAttributes.Directory);
			bool isReparsePoint = InteropHelper.HasFileAttribute(path, System.IO.FileAttributes.ReparsePoint);
			...
			if (itemType is null || isShortcut || isHiddenItem || isReparsePoint)
			{
				...
				{
					if (!isDirectory &&
						Core.Helpers.InteropHelper.GetWin32FindDataForPath(path, out var findData) &&
						findData.dwReserved0 == InteropHelper.IO_REPARSE_TAG_SYMLINK)
					{
						shortcutInfo.TargetPath = InteropHelper.ParseSymLink(path);
					}
					itemType ??= isDirectory ? FilesystemItemType.Directory : FilesystemItemType.File;
				}
				else if (isHiddenItem)
				{
					itemType = InteropHelper.HasFileAttribute(path, System.IO.FileAttributes.Directory) ? FilesystemItemType.Directory : FilesystemItemType.File;
				}
				else
				{
