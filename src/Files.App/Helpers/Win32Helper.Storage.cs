...
namespace Files.App.Helpers
{
	...
	public static partial class Win32Helper
	{
		...
		public static SafeFileHandle CreateFileForWrite(string filePath, bool overwrite = true)
		{
			return new SafeFileHandle(
				CreateFileFromApp(
					...
					GENERIC_WRITE,
					0,
					IntPtr.Zero, overwrite ? CREATE_ALWAYS : OPEN_ALWAYS,
		...
		public static SafeFileHandle OpenFileForRead(string filePath, bool readWrite = false, uint flags = 0)
		{
			return new SafeFileHandle(
				CreateFileFromApp(
					...
					IntPtr.Zero,
					OPEN_EXISTING, (uint)File_Attributes.BackupSemantics | flags, IntPtr.Zero),
				true);
		}

		public static bool GetFileDateModified(string filePath, out FILETIME dateModified)
		{
			using var hFile = new SafeFileHandle(
				CreateFileFromApp(
					filePath,
					GENERIC_READ,
					FILE_SHARE_READ,
		...
		{
			if (IntPtr.Size == 4)
				return SetWindowLongPtr32((nint)hWnd, (int)nIndex, dwNewLong);
			else
