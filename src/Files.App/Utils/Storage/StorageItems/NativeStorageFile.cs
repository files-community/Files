	...
	public class NativeStorageFile : BaseStorageFile
	{
		...
		public override IAsyncOperation<BaseStorageFile> CopyAsync(IStorageFolder destinationFolder, string desiredNewName, NameCollisionOption option)
		{
			return AsyncInfo.Run<BaseStorageFile>(async (cancellationToken) =>
			{
				...
				{
					if (!await Task.Run(() => InteropHelper.CopyFileFromApp(Path, destination, option != NameCollisionOption.ReplaceExisting)))
					{
		...
		{
			using var hFile = InteropHelper.CreateFileForWrite(Path, false);
			if (hFile.IsInvalid)
			{
				throw new Win32Exception(Marshal.GetLastWin32Error());
			}
		...
		public override IAsyncAction DeleteAsync()
		{
			...
			{
				if (!InteropHelper.DeleteFileFromApp(Path))
				{
					throw new Win32Exception(Marshal.GetLastWin32Error());
				}
			});
		...
		{
			using var hFile = InteropHelper.OpenFileForRead(path);
		...
		public override IAsyncAction MoveAsync(IStorageFolder destinationFolder, string desiredNewName, NameCollisionOption option)
		{
			return AsyncInfo.Run(async (cancellationToken) =>
			{
				...
				if (!IsAlternateStream)
				{
					if (!await Task.Run(() => InteropHelper.MoveFileFromApp(Path, destination)))
					{
						throw new Win32Exception(Marshal.GetLastWin32Error());
					}
				}
		...
		{
			var hFile = InteropHelper.OpenFileForRead(Path, accessMode == FileAccessMode.ReadWrite);
