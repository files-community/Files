...
namespace Files.App.Utils.Storage
{
	public static class Win32StorageEnumerator
	{
		...
		public static async Task<List<ListedItem>> ListEntries(
			string path,
			IntPtr hFile,
			Core.Helpers.Win32Interop.WIN32_FIND_DATA findData,
		...
		{
			foreach (var ads in Win32InteropHelper.GetAlternateStreams(itemPath))
		...
		)
		{
			...
			try
			{
				FileTimeToSystemTime(ref findData.ftLastWriteTime, out Core.Helpers.Win32Interop.SYSTEMTIME systemModifiedTimeOutput);
				itemModifiedDate = systemModifiedTimeOutput.ToDateTime();

				FileTimeToSystemTime(ref findData.ftCreationTime, out Core.Helpers.Win32Interop.SYSTEMTIME systemCreatedTimeOutput);
		...
		)
		{
			...
			try
			{
				FileTimeToSystemTime(ref findData.ftLastWriteTime, out Core.Helpers.Win32Interop.SYSTEMTIME systemModifiedDateOutput);
				itemModifiedDate = systemModifiedDateOutput.ToDateTime();

				FileTimeToSystemTime(ref findData.ftCreationTime, out Core.Helpers.Win32Interop.SYSTEMTIME systemCreatedDateOutput);
				itemCreatedDate = systemCreatedDateOutput.ToDateTime();

				FileTimeToSystemTime(ref findData.ftLastAccessTime, out Core.Helpers.Win32Interop.SYSTEMTIME systemLastAccessOutput);
			...
			bool isSymlink = isReparsePoint && findData.dwReserved0 == Win32InteropHelper.IO_REPARSE_TAG_SYMLINK;

			if (isSymlink)
			{
				var targetPath = Win32InteropHelper.ParseSymLink(itemPath);
