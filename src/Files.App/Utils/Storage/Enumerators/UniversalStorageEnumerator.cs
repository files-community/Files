...
namespace Files.App.Utils.Storage
{
	public static class UniversalStorageEnumerator
	{
			...
			if (!cancellationToken.IsCancellationRequested)
			{
				...
				else if (folder is BinStorageFolder binFolder)
				{
					return new RecycleBinItem(folder.FolderRelativeId)
					{
						...
						FileImage = null,
						LoadFileIcon = false,
						ItemPath = string.IsNullOrEmpty(folder.Path) ? StoragePathHelper.Combine(currentStorageFolder.Path, folder.Name) : folder.Path,
			...
			CancellationToken cancellationToken)
		{
			...
			var itemPath = string.IsNullOrEmpty(file.Path) ? StoragePathHelper.Combine(currentStorageFolder.Path, file.Name) : file.Path;
