...
namespace Files.App.Utils
{
	public static class WSLDistroManager
	{
		...
		private static readonly List<LocatableWSLItem> distros = new();
		public static IReadOnlyList<LocatableWSLItem> Distros
		{
			get
			{
				lock (distros)
		...
		public static async Task UpdateDrivesAsync()
		{
			try
			{
				...
				foreach (StorageFolder folder in await distroFolder.GetFoldersAsync())
				{
					...
					var distro = new LocatableWSLItem
					{
		...
		public static bool TryGetDistro(string path, [NotNullWhen(true)] out LocatableWSLItem? distro)
		{
