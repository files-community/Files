...
namespace Files.App.Utils.Git
{
	internal sealed partial class LibGit2 : IVersionControl
	{
		...
		public async Task<BranchItem[]> GetBranchesNames(string? path)
		{
			if (string.IsNullOrWhiteSpace(path) || !IsRepoValid(path))
				return [];
		...
		public async Task PullOriginAsync(string? repositoryPath)
		{
			...
			var result = await DoGitOperationAsync<GitOperationResult>(() =>
			{
				try
				{
					LibGit2Sharp.Commands.Pull(repository, signature, _pullOptions);
				}
				catch (Exception ex)
				{
		...
		public async Task PushToOriginAsync(string? repositoryPath, string? branchName)
		{
			...
			try
			{
				...
				if (!branch.IsTracking)
				{
					...
					repository.Branches.Update(branch, b => b.Remote = origin.Name, b => b.UpstreamBranch = branch.CanonicalName);
				}
		...
		public bool IsRepositoryEx(string path, out string repoRootPath)
		{
			repoRootPath = path;
		...
		public GitItemModel GetGitInformationForItem(Repository repository, string path, bool getStatus = true, bool getCommit = true)
		{
		...
		public bool IsValidRepoUrl(string url) => GitHubRepositoryRegex().IsMatch(url);
