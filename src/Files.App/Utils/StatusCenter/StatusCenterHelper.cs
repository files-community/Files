	...
	public static class StatusCenterHelper
	{
			...
			long totalSize = 0)
		{
			...
			if (source is not null && source.Any())
				sourceDir = StoragePathHelper.GetParentDir(source.First().Path);

			if (destination is not null && destination.Any())
				destinationDir = StoragePathHelper.GetParentDir(destination.First());
		...
		public static StatusCenterItem AddCard_Move(
			...
			long totalSize = 0)
		{
			var sourceDir = StoragePathHelper.GetParentDir(source.FirstOrDefault()?.Path);
			var destinationDir = StoragePathHelper.GetParentDir(destination.FirstOrDefault());
			...
			long totalSize = 0)
		{
			...
			var sourceDir = StoragePathHelper.GetParentDir(source.FirstOrDefault());
			var destinationDir = StoragePathHelper.GetParentDir(destination.FirstOrDefault());
		...
		public static void UpdateCardStrings(StatusCenterItem card)
		{
			...
			if (card.Source is not null && card.Source.Any())
			{
				...
				{
					sourcePath = StoragePathHelper.GetParentDir(card.Source.First());
			...
			{
				destinationPath = StoragePathHelper.GetParentDir(card.Destination.First());
