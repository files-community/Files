...
namespace Files.App.Utils.FileTags
{
	public static class FileTagsHelper
	{
		...
		{
			var tagString = Helpers.Win32Helper.ReadStringFromFile($"{filePath}:files");
		...
		{
			var isDateOk = Helpers.Win32Helper.GetFileDateModified(filePath, out var dateModified); // Backup date modified
			var isReadOnly = Helpers.Win32Helper.HasFileAttribute(filePath, IO.FileAttributes.ReadOnly);
			if (isReadOnly) // Unset read-only attribute (#7534)
			{
				Helpers.Win32Helper.UnsetFileAttribute(filePath, IO.FileAttributes.ReadOnly);
			}
			...
			{
				var result = Helpers.Win32Helper.WriteStringToFile($"{filePath}:files", string.Join(',', tag));
				if (result == false)
				{
			...
			{
				Helpers.Win32Helper.SetFileAttribute(filePath, IO.FileAttributes.ReadOnly);
			}
			if (isDateOk)
			{
				Helpers.Win32Helper.SetFileDateModified(filePath, dateModified); // Restore date modified
			}
		}
		...
		public static void UpdateTagsDb()
		{
			...
			{
				var pathFromFrn = Shell.Win32Helper.PathFromFileId(file.Frn ?? 0, file.FilePath);
		...
		public static ulong? GetFileFRN(string filePath) => Helpers.Win32Helper.GetFileFRN(filePath);
