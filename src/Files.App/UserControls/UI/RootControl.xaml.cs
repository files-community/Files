...
namespace Files.App.UserControls.UI
{
	public sealed partial class RootControl : UserControl, IAsyncInitialize
	{
				...
				case ILaunchActivatedEventArgs launchArgs:
						...
						|| CommandLineParser.SplitArguments(launchArgs.Arguments, true)[0].EndsWith($"files", StringComparison.OrdinalIgnoreCase)))
					{
						...
						if (ppm.IsEmpty())
							...
							return await InitializeFromCmdLineArgsAsync(ppm, mainPageViewModel);
					}
		...
		private async Task<object?> InitializeFromCmdLineArgsAsync(ParsedCommands parsedCommands, string activationPath = "")
		{
			async Task<object?> PerformNavigationAsync(string payload, string selectItem = null)
			{
				...
				if (MainPageViewModel.AppInstances.Any())
				{
					InteropHelpers.SwitchToThisWindow(MainWindow.Instance.WindowHandle, true);
			...
			foreach (var command in parsedCommands)
			{
				switch (command.Type)
				{
					...
					case ParsedCommandType.ExplorerShellCommand:
						...
						return await PerformNavigationAsync(command.Payload, selectItemCommand?.Payload);

					case ParsedCommandType.SelectItem:
						if (SystemIO.Path.IsPathRooted(command.Payload))
							return await PerformNavigationAsync(SystemIO.Path.GetDirectoryName(command.Payload), SystemIO.Path.GetFileName(command.Payload));
						break;
					...
					case ParsedCommandType.Unknown:
						...
						{
							return await PerformNavigationAsync(activationPath);
						}
						else
						{
							if (!string.IsNullOrEmpty(command.Payload))
							{
								...
								return await PerformNavigationAsync(target);
							}
							else
							{
								return await PerformNavigationAsync(null);
							}
						}
						break;
