...
namespace Files.App.Data.Items
{
	public sealed partial class DriveItem : ExpandableSidebarItemBase, INavigationControlItem, IFolder, IExpandableSidebarFolder
	{
		...
		public async Task UpdateLabelAsync()
		{
			...
			{
				var root = Root!;
				var properties = await root.Properties.RetrievePropertiesAsync(new List<string> { "System.ItemNameDisplay" })
		...
		public async Task UpdatePropertiesAsync()
		{
			try
			{
				...
				var properties = await root.Properties.RetrievePropertiesAsync(new List<string> { "System.FreeSpace", "System.Capacity", "System.Volume.FileSystem" })
					.AsTask().WithTimeoutAsync(TimeSpan.FromSeconds(5));
