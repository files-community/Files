...
namespace Files.App.ViewModels.Properties
{
	public class CompatibilityProperties : ObservableObject
	{
		...
		public List<LocalizedEnumHelper<HighDpiOption>> HighDpiOptionList { get; } =
			Enum.GetValues(typeof(HighDpiOption)).Cast<HighDpiOption>().Select(x => new LocalizedEnumHelper<HighDpiOption>(x)).ToList();

		public List<LocalizedEnumHelper<HighDpiOverride>> HighDpiOverrideList { get; } =
			Enum.GetValues(typeof(HighDpiOverride)).Cast<HighDpiOverride>().Where(x => x != Shared.HighDpiOverride.Advanced).Select(x => new LocalizedEnumHelper<HighDpiOverride>(x)).ToList();

		public List<LocalizedEnumHelper<OSCompatibility>> OSCompatibilityList { get; } =
			Enum.GetValues(typeof(OSCompatibility)).Cast<OSCompatibility>().Select(x => new LocalizedEnumHelper<OSCompatibility>(x)).ToList();

		public List<LocalizedEnumHelper<ReducedColorMode>> ReducedColorModeList { get; } =
			Enum.GetValues(typeof(ReducedColorMode)).Cast<ReducedColorMode>().Select(x => new LocalizedEnumHelper<ReducedColorMode>(x)).ToList();
