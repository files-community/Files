...
namespace Files.App.ViewModels.LayoutModes
{
	...
	public abstract class BaseLayoutViewModel : ObservableObject, IDisposable, IBaseLayoutViewModel
	{
		...
		public BaseLayoutViewModel()
		{
			...
			jumpTimer = MainWindow.Instance.DispatcherQueue.CreateTimer();
			...
			hoverTimer = MainWindow.Instance.DispatcherQueue.CreateTimer();
		}
		...
		public async void BaseContextFlyout_Opening(object? sender, object e)
		{
			...
			try
			{
				...
				var items = ContextFlyoutItemHelper.GetItemContextCommandsWithoutShellItems(
					...
					selectedItems: new List<ListedItem> { ParentShellPageInstance!.FilesystemViewModel.CurrentFolder },
		...
		private async Task ItemPointerPressed(PointerRoutedEventArgs e)
		{
			...
			{
				_associatedInstance.SlimContentPage.BaseViewModel.IsMiddleClickToScrollEnabled = false;
				_associatedInstance.SlimContentPage.BaseViewModel.IsMiddleClickToScrollEnabled = true;
