...
namespace Files.App.ViewModels.Layouts
{
	...
	public sealed class BaseLayoutViewModel : IDisposable
	{
		...
		public async Task DragOverAsync(DragEventArgs e)
		{
			...
			if (_associatedInstance.CurrentShellViewModel.IsPageTypeSearchResults)
			{
			...
			if (FilesystemHelpers.HasDraggedStorageItems(e.DataView))
			{
				...
				if (_associatedInstance.CurrentShellViewModel.IsPageTypeSearchResults || draggedItems.Any() && draggedItems.AreItemsAlreadyInFolder(_associatedInstance.ShellViewModel.WorkingDirectory))
				{
					e.AcceptedOperation = DataPackageOperation.None;
				}
