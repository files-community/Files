	...
	public class BaseLayoutViewModel : IDisposable
	{
		...
		private async Task ItemPointerPressedAsync(PointerRoutedEventArgs e)
		{
				...
				Item.PrimaryItemAttribute == StorageItemTypes.Folder)
			{
				...
				if (Item.IsShortcut)
					await NavigationHelper.OpenPathInNewTab(((e.OriginalSource as FrameworkElement)?.DataContext as ShortcutItem)?.TargetPath ?? Item.ItemPath);
				else
					await NavigationHelper.OpenPathInNewTab(Item.ItemPath);
			}
		}
