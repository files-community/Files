...
namespace Files.App.ViewModels.Settings
{
	public class AdvancedViewModel : ObservableObject
	{
		...
		private async Task SetAsDefaultExplorerAsync()
		{
			...
			var destFolder = Path.Combine(ApplicationData.Current.LocalFolder.Path, "Settings");
			Directory.CreateDirectory(destFolder);

			foreach (var file in Directory.GetFiles(Path.Combine(Package.Current.InstalledLocation.Path, "Files.App", "Assets", "Settings")))
			{
		...
		private async Task SetAsOpenFileDialogAsync()
		{
			...
			var destFolder = Path.Combine(ApplicationData.Current.LocalFolder.Path, "Libraries");
			Directory.CreateDirectory(destFolder);
			foreach (var file in Directory.GetFiles(Path.Combine(Package.Current.InstalledLocation.Path, "Files.App", "Assets", "Libraries")))
			{
				if (!SafetyExtensions.IgnoreExceptions(() => File.Copy(file, Path.Combine(destFolder, Path.GetFileName(file)), true), App.Logger))
				{
					// Error copying files
