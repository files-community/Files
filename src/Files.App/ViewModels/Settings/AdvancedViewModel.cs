...
namespace Files.App.ViewModels.Settings
{
	public class AdvancedViewModel : ObservableObject
	{
		...
		private async Task SetAsDefaultExplorerAsync()
		{
			...
			{
				if (!await Utils.Shell.Win32Helper.RunPowerShellCommandAsync($"-command \"New-Item -Force -Path '{dataPath}' -ItemType Directory; Copy-Item -Filter *.* -Path '{destFolder}\\*' -Recurse -Force -Destination '{dataPath}'\"", false))
				{
					// Error copying files
					await DetectResult();
					return;
			...
			{
				await Utils.Shell.Win32Helper.RunPowerShellCommandAsync($"-command \"Remove-Item -Path '{dataPath}' -Recurse -Force\"", false);
			}
