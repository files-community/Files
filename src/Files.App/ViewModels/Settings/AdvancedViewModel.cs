...
namespace Files.App.ViewModels.Settings
{
	public sealed class AdvancedViewModel : ObservableObject
	{
		...
		private async Task SetAsDefaultExplorerAsync()
		{
			...
			{
				if (!await Win32Helper.RunPowershellCommandAsync($"-command \"New-Item -Force -Path '{dataPath}' -ItemType Directory; Copy-Item -Filter *.* -Path '{destFolder}\\*' -Recurse -Force -Destination '{dataPath}'\"", PowerShellOptions.Hidden))
				{
					// Error copying files
					await DetectResult();
					return;
			...
			{
				await Win32Helper.RunPowershellCommandAsync($"-command \"Remove-Item -Path '{dataPath}' -Recurse -Force\"", PowerShellOptions.Hidden);
			}
