	...
	public abstract class BaseLayout : Page, IBaseLayout, INotifyPropertyChanged
	{
			...
			try
			{
				...
				if (!InstanceViewModel!.IsPageTypeSearchResults && !InstanceViewModel.IsPageTypeZipFolder)
				{
					...
					if (shellMenuItems.Any())
						await AddShellItemsToMenu(shellMenuItems, BaseContextMenuFlyout, shiftPressed);
					else
		...
		private async Task LoadMenuItemsAsync()
		{
			...
			if (!InstanceViewModel.IsPageTypeZipFolder)
			{
				...
				if (shellMenuItems.Any())
					await AddShellItemsToMenu(shellMenuItems, ItemContextMenuFlyout, shiftPressed);
				else
		...
		private async Task AddShellItemsToMenu(List<ContextMenuFlyoutItemViewModel> shellMenuItems, CommandBarFlyout contextMenuFlyout, bool shiftPressed)
		{
