...
namespace Files.App.Dialogs
{
	public sealed partial class ReorderSidebarItemsDialog : ContentDialog, IDialog<ReorderSidebarItemsDialogViewModel>
	{
		...
		{
			if ((sender as Grid)?.DataContext is not SideBarLocationItem locationItem)
				return;
			var deferral = e.GetDeferral();
			
			if ((e.DataView.Properties["sourceLocationItem"] as Grid)?.DataContext is SideBarLocationItem sourceLocationItem)
			{
		...
		private void DragOver_SetCaptions(SideBarLocationItem senderLocationItem, SideBarLocationItem sourceLocationItem, DragEventArgs e)
		{
		...
		{
			if (sender is not Grid navView || navView.DataContext is not SideBarLocationItem locationItem)
				return;
