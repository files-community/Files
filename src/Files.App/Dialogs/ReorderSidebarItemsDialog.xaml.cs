...
namespace Files.App.Dialogs
{
	public sealed partial class ReorderSidebarItemsDialog : ContentDialog, IDialog<ReorderSidebarItemsDialogViewModel>
	{
		...
		{
			if (sender is not Grid nav || nav.DataContext is not LocatableStorageItem)
				return;
		...
		{
			if ((sender as Grid)?.DataContext is not LocatableStorageItem locationItem)
				return;
			var deferral = e.GetDeferral();
			
			if ((e.DataView.Properties["sourceLocationItem"] as Grid)?.DataContext is LocatableStorageItem sourceLocationItem)
			{
		...
		private void DragOver_SetCaptions(LocatableStorageItem senderLocationItem, LocatableStorageItem sourceLocationItem, DragEventArgs e)
		{
		...
		{
			if (sender is not Grid navView || navView.DataContext is not LocatableStorageItem locationItem)
				return;
