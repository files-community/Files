...
namespace Files.Core.SourceGenerator.Utilities.LightJson
{
	...
	internal struct JsonValue
	{
		...
		public DateTime? AsDateTime => IsString && DateTime.TryParse((string)reference, out var value) ? value : null;
...
=>jsonValue.IsDateTime || jsonValue.IsNull ? jsonValue.AsDateTime : null;
        ...
        private class JsonValueDebugView
        {
			...
			public object Value => jsonValue.IsJsonObject
						? (JsonObject)jsonValue.reference
						: jsonValue.IsJsonArray ? (JsonArray)jsonValue.reference : (object)jsonValue;
		}
    }
}
