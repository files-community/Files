...
namespace Files.App.Storage
{
	...
	public unsafe partial class WindowsBulkOperations : IDisposable
	{
		...
		private readonly IFileOperationProgressSink _progressSink;
		...
		public WindowsBulkOperations(HWND ownerHWnd = default, FILEOPERATION_FLAGS flags = FILEOPERATION_FLAGS.FOF_ALLOWUNDO | FILEOPERATION_FLAGS.FOF_NOCONFIRMMKDIR)
		{
			HRESULT hr = PInvoke.CoCreateInstance(typeof(FileOperation).GUID, null, CLSCTX.CLSCTX_LOCAL_SERVER, out IFileOperation? fileOperation);
			...
			_progressSink = new WindowsBulkOperationsSink(this);
			hr = _fileOperation.Advise(_progressSink, out var progressSinkCookie).ThrowIfFailedOnDebug();
		...
		public unsafe HRESULT QueueCopyOperation(WindowsStorable targetItem, WindowsFolder destinationFolder, string? copyName)
		{
			fixed (char* pszCopyName = copyName)
				return _fileOperation.CopyItem(targetItem.ThisPtr, destinationFolder.ThisPtr, pszCopyName, _progressSink);
		}
		...
		{
			return _fileOperation.DeleteItem(targetItem.ThisPtr, _progressSink);
		}
		...
		public unsafe HRESULT QueueCreateOperation(WindowsFolder destinationFolder, FILE_FLAGS_AND_ATTRIBUTES fileAttributes, string name, string? templateName)
		{
			fixed (char* pszName = name, pszTemplateName = templateName)
				return _fileOperation.NewItem(destinationFolder.ThisPtr, (uint)fileAttributes, pszName, pszTemplateName, _progressSink);
		}
		...
		{
			fixed (char* pszNewName = newName)
				return _fileOperation.RenameItem(targetItem.ThisPtr, pszNewName, _progressSink);
		}
