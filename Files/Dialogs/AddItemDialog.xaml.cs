    ...
    public sealed partial class AddItemDialog : ContentDialog
    {
        ...
        public static async void CreateFile(AddItemType fileType)
        {
            ...
            if (fileType == AddItemType.Folder)
            {
                ...
                TabInstance.ViewModel.AddFileOrFolder(new ListedItem(folder.FolderRelativeId) { FileName = folder.DisplayName, FileDateReal = DateTimeOffset.Now, LoadUnknownTypeGlyph = false, LoadFolderGlyph = true, LoadFileIcon = false, FileType = "Folder", FileImage = null, FilePath = folder.Path });
            }
            else if (fileType == AddItemType.TextDocument)
            {
                ...
                TabInstance.ViewModel.AddFileOrFolder(new ListedItem(item.FolderRelativeId) { FileName = item.DisplayName, FileDateReal = DateTimeOffset.Now, LoadUnknownTypeGlyph = true, LoadFolderGlyph = false, LoadFileIcon = false, FileType = item.DisplayType, FileImage = null, FilePath = item.Path, DotFileExtension = item.FileType });
            }
